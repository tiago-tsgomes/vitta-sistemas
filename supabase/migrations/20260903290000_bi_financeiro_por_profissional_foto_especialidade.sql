-- Ajusta bi_financeiro_por_profissional (categoria 3) para o novo layout de
-- linha do card "Por profissional" em bi/pages/financeiro.html, a pedido do
-- usuário em 03/09/2026 (mockup: número do ranking + foto do profissional +
-- nome/especialidade à esquerda, valor + repasse/líquido à direita).
--
-- Adiciona 2 campos no retorno de cada profissional:
--   - foto: PROFISSIONAL.FOTO_PROF (mesma coluna usada em pages/profissionais.html)
--   - especialidade: PROFISSIONAL.ESPECIAL_PROF (texto livre, mesma coluna
--     usada como "especialidade" em pages/profissionais.html)
--
-- CREATE OR REPLACE não muda a lista de parâmetros aqui (continua
-- (uuid, text, date, date)), então não precisa de drop function antes.
