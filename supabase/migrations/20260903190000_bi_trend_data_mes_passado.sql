-- Adiciona o período "mes_passado" ao filtro de Tendências do Vitta BI:
-- mesmo bucket semanal (4 semanas) usado em "mes", mas referente ao mês
-- anterior completo em vez do mês corrente.
  ...
  if p_period = 'mes' then
    ...
    for v_n in 0..3 loop
      insert into _bi_buckets values (
        v_n,
        'Sem ' || (v_n + 1),
        v_mes_anterior + (v_n * 7),
        case when v_n = 3 then v_mes_atual
             else v_mes_anterior + ((v_n + 1) * 7) end
      );
    end loop;
