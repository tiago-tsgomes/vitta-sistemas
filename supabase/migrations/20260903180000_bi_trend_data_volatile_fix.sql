-- bi_trend_data estava marcada como STABLE, mas cria uma tabela temporária
-- (_bi_buckets) internamente para montar os períodos — isso é uma escrita.
-- O PostgREST executa funções STABLE/IMMUTABLE dentro de uma transação
-- somente leitura, então toda chamada falhava com "cannot execute CREATE
-- TABLE in a read-only transaction" e os gráficos de Tendências ficavam
-- vazios (sem erro visível na tela, só no console). Removendo STABLE
-- (volta para VOLATILE, o padrão) resolve — a função não é chamada em
-- contexto que dependa da otimização de STABLE.
